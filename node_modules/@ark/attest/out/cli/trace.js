...
import { TsServer, getDescendants, getStringifiableType, nearestBoundingCallExpression } from "../cache/ts.js";
 ...
 */
const formatMillis = (ms, fractionDigits = 2) => (ms / 1000).toFixed(fractionDigits);
/**
 * Formats microseconds to a padded string of milliseconds.
 */
const formatPaddedMillis = (ms, pad, fractionDigits = 2) => formatMillis(ms, fractionDigits).padStart(pad);
...
class ProgressDisplay {
    ...
    startTime = 0;
    timerId;
    barWidth = 30; // Width of the progress bar itself
    ...
    update(processedCount) {
        ...
        if (this.processedItems >= this.totalItems)
            this.stop(); // Automatically stop if all items are processed
    }
    render() {
...
export const trace = async (args) => {
    ...
    if (!existsSync(traceFile)) {
        outputCapture.write(`❌ No trace data found (expected a file at ${traceFile}). TSC output:\n${tracingOutput}`);
...
const generateTraceData = (traceDir, tsconfigPath, packageDir) => {
    try {
        const output = getShellOutput(`${baseDiagnosticTscCmd} --project ${tsconfigPath} --generateTrace ${traceDir}`, { cwd: packageDir });
...
const initializeAnalysisContext = (traceDir) => {
    const tsServer = TsServer.instance;
    const tracePath = join(traceDir, "trace.json");
    if (!existsSync(tracePath)) {
        throw new Error(`Critical: Expected a trace file at ${tracePath}, but it was not found during context initialization.`);
    }
    const traceEntries = readJson(tracePath);
    return {
        traceDir,
        tsServer,
        traceEntries,
...
const filterDurationEntries = (ctx) => {
    ...
    outputCapture.write(`Found ${ctx.durationEntries.length} complete event traces with duration, path, and position.`);
};
const processDurationEntry = (entry, ctx) => {
    const entryPath = entry.args.path;
    const entryPos = entry.args.pos;
    const entryEnd = entry.args.end;
    const entryDur = entry.dur;
    ...
    if (callExpr) {
        ...
        const relevantNode = findMostSpecificNodeInRange(sourceFile, entryPos, entryEnd);
        if (relevantNode) {
            ...
            const typeNodeIdPart = nodeType.id ?? nodeType.toString().substring(0, 20);
            callRangeData = {
    ...
    if (callRangeData) {
        ...
        throw new Error(`Failed to identify a processable AST node for duration entry (name: ${entry.name}, path: ${entryPath}, pos: ${entryPos}-${entryEnd}).`);
    }
};
const findCallExpressionInRange = (file, start, end) => {
    ...
    const nodesInRange = allNodes.filter(node => node.pos >= start && node.end <= end);
    const callExpressions = nodesInRange.filter(node => ts.isCallExpression(node));
    if (callExpressions.length === 0)
        return undefined;
    const methodCalls = callExpressions.filter(call => ts.isPropertyAccessExpression(call.expression));
    return methodCalls.length > 0 ? methodCalls[0] : callExpressions[0];
};
const extractFunctionName = (callExpr) => {
...
const buildCallTree = (ctx) => {
    // This message goes to outputCapture for the summary file
    outputCapture.write(`Building call tree from ${ctx.callRanges.length} ranges...`);
    ctx.callRanges.sort((a, b) => a.startTime - b.startTime);
    const activeCallStack = [];
    for (const call of ctx.callRanges) {
        while (activeCallStack.length > 0 &&
    ...
    outputCapture.write(`Call tree built with ${ctx.rootCalls.length} root calls.`);
};
const calculateSelfTimes = (call) => {
    let childrenTime = 0;
    for (const child of call.children)
        childrenTime += calculateSelfTimes(child);
    call.selfTime = call.duration - childrenTime;
    if (call.selfTime < 0)
        call.selfTime = 0;
    return call.duration;
};
const collectFunctionStats = (call, ctx) => {
    ...
    if (isNaN(pos) || isNaN(end)) {
        throw new Error(`Invalid position in callSite: ${call.callSite}. Pos: ${posStr}, End: ${endStr}`);
    }
    ...
    stats.count++;
    for (const child of call.children)
        collectFunctionStats(child, ctx);
};
const sortAndRankFunctions = (ctx) => {
    ...
    ctx.allFunctions = Object.values(ctx.functionStats).sort((a, b) => b.selfTime - a.selfTime);
};
const analyzeTypeInstantiations = (traceDir) => {
    ...
    for (const root of ctx.rootCalls)
    ...
    if (ctx.callRanges.length > 0) {
        ...
        writeToCsv(rangesCsvPath, ["Function Name", "Self (ms)", "Duration (ms)", "Location"], ungroupedStats.map(stat => [
            stat.functionName,
            formatMillis(stat.selfTime, 3),
            formatMillis(stat.duration, 3),
            formatLocation(stat.callSite)
        ]));
        writeToCsv(namesCsvPath, [
            "Function Name",
            "Total Self (ms)",
            "Avg Self (ms)",
            "Call Count",
...
const collectUngroupedStats = (ctx) => {
    ...
    const flattenCallTree = (call) => {
        ...
        for (const child of call.children)
            flattenCallTree(child);
    };
    for (const root of ctx.rootCalls)
        flattenCallTree(root);
    return ungroupedStats.sort((a, b) => b.selfTime - a.selfTime);
};
const displayIndividualSummary = (stats) => {
    ...
    for (let i = 0; i < topN; i++) {
        ...
        outputCapture.write(`${(i + 1).toString().padStart(4)} | ${typeNameFormatted} | ${selfTimeMs} | ${location}`);
    }
};
...
const displayGroupedSummary = (functions) => {
    displayTableHeader([
        "Rank",
        "Function Name",
        "Total Self (ms)",
    ...
    for (let i = 0; i < topN; i++) {
        ...
        const avgTimeMs = formatPaddedMillis(stats.selfTime / Math.max(1, stats.count), 13, 2);
        ...
        const topUsageLocation = formatLocation(`${topUsage.path}:${topUsage.pos}-${topUsage.end}`);
        outputCapture.write(`${(i + 1).toString().padStart(4)} | ${typeNameFormatted} | ${totalTimeMs} | ${avgTimeMs} | ${calls} | ${topUsageLocation} (${topUsageTime})`);
    }
};
const formatTypeName = (typeName, maxLength) => {
    typeName = typeName.replace(/\(\)$/, "");
    if (typeName.length <= maxLength)
        return typeName.padEnd(maxLength);
    const charsToKeep = maxLength - 3;
    const firstPart = Math.ceil(charsToKeep * 0.7);
    const lastPart = charsToKeep - firstPart;
    return (typeName.substring(0, firstPart) +
        "..." +
        (lastPart > 0 ? typeName.substring(typeName.length - lastPart) : "")).padEnd(maxLength);
};
...
const findMostSpecificNodeInRange = (file, start, end) => {
    const allNodes = getDescendants(file);
    const nodesInRange = allNodes.filter(node => node.pos >= start && node.end <= end);
    if (nodesInRange.length === 0)
        return undefined;
    const leafNodes = nodesInRange.filter(node => !nodesInRange.some(other => other !== node && other.pos >= node.pos && other.end <= node.end));
    return findNodeByPreference(leafNodes.length > 0 ? leafNodes : nodesInRange);
};
const findNodeByPreference = (nodes) => {
    if (nodes.length === 0)
        return undefined;
    const typeReference = nodes.find(node => ts.isTypeReferenceNode(node) || ts.isTypeQueryNode(node));
    if (typeReference)
        return typeReference;
    const declaration = nodes.find(node => ts.isVariableDeclaration(node) ||
    ...
    const assignment = nodes.find(node => ts.isBinaryExpression(node) &&
        node.operatorToken.kind === ts.SyntaxKind.EqualsToken);
    if (assignment)
        return assignment;
    const expression = nodes.find(node => ts.isExpressionStatement(node) || ts.isExpression(node));
    if (expression)
        return expression;
    return nodes.sort((a, b) => a.end - a.pos - (b.end - b.pos))[0];
};
const writeToCsv = (filePath, headers, rows) => {
    const content = [
        headers.join(","),
